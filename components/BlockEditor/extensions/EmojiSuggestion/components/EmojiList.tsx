...
const EmojiList = forwardRef(
  (props: EmojiListProps, ref: ForwardedRef<{ onKeyDown: (_evt: SuggestionKeyDownProps) => boolean }>) => {
    ...
    return (
      <Panel className="max-h-72 max-w-72 overflow-y-auto">
        {props.items.map((item: EmojiItem, index: number) => (
          ...
          >
            {item.fallbackImage ? <img src={item.fallbackImage} className="size-5" alt="emoji" /> : item.emoji}{" "}
